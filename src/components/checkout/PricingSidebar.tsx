...
const PricingSidebar: React.FC<PricingSidebarProps> = ({
  ...
  const discountedPrice =
    cartValidation?.cartSummary?.totalDiscountedPrice ||
    ...
    <div className="mt-8 lg:mt-0 space-y-6">
              ...
              <div className="bg-red-50 border border-red-200 rounded-lg p-3 mb-3">
                ...
                <div className="text-xs text-red-600 space-y-1">
                  {cartValidation.cartSummary.hasOutOfStockItems && (
                    <p>
                      • {cartValidation.recommendations?.outOfStockCount || 0}{" "}
                      out of stock item(s)
                    </p>
                  )}
                  {cartValidation.cartSummary.hasQuantityIssues && (
                    <p>
                      •{" "}
                      {cartValidation.recommendations?.quantityIssuesCount || 0}{" "}
                      quantity issue(s)
                    </p>
                  )}
                  {cartValidation.cartSummary.hasLowStockWarnings && (
                    <p>
                      • {cartValidation.recommendations?.lowStockCount || 0} low
                      stock warning(s)
                    </p>
                  )}
                </div>
          ...
          {pricingDetails.savings > 0 && (
            <div className="bg-green-50 border border-green-200 rounded-lg p-3">
              <div className="flex items-center justify-between">
                <p className="text-green-700 font-medium">You're Saving:</p>
          ...
          {invalidItemsCount > 0 && (
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-3">
              <p className="text-gray-600 text-sm text-center">
                {invalidItemsCount} item{invalidItemsCount > 1 ? "s" : ""}{" "}
                excluded from total
              </p>
            </div>
          )}
      ...
      <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl p-6 border border-green-200">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <p className="font-bold text-green-800">100% Secure Checkout</p>
      ...
      <div className="bg-white rounded-xl shadow-lg p-6 border border-emerald-100">
        ...
        <div className="space-y-3">
          ...
          <p className="text-xs text-gray-500">Available 24/7 to help you</p>
        </div>
      </div>
      ...
      {isValidatingCart && (
        <div className="bg-white rounded-xl shadow-lg p-6 border border-emerald-100">
          <div className="flex items-center justify-center space-x-3">
            ...
            <span className="text-sm text-gray-600">Validating cart...</span>
          </div>
        </div>
      )}
    </div>
